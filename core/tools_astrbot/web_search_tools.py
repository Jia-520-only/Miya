...
class _KeyRotator:
    ...
    async def get(self, provider_settings: dict) -> str:
        ...
        if not keys:
            raise ValueError(f"Error: {self.provider_name} API key is not configured in AstrBot.")
...
def normalize_legacy_web_search_config(cfg) -> None:
    ...
    changed = False
    if provider_settings.get("websearch_provider") == "default" and provider_settings.get("web_search", False):
