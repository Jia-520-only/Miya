...
class FutureTaskTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(self, context: ContextWrapper[AstrAgentContext], **kwargs) -> ToolExecResult:
        ...
        if action == "create":
            ...
            suffix = f"one-time at {next_run}" if run_once else f"expression '{cron_expression}' (next {next_run})"
        ...
        if action == "edit":
            ...
            if not any(key in kwargs for key in ("name", "note", "run_once", "cron_expression", "run_at")):
            ...
            run_once = bool(kwargs["run_once"]) if "run_once" in kwargs else bool(job.run_once)
            cron_expression = (
                str(kwargs.get("cron_expression") or "").strip() if "cron_expression" in kwargs else job.cron_expression
            )
            cron_expression = cron_expression or None

            try:
                run_at_dt = _parse_run_at(kwargs.get("run_at")) if "run_at" in kwargs else _parse_run_at(current_run_at)
            except Exception:
                return "error: run_at must be ISO datetime, e.g., 2026-02-02T08:00:00+08:00"
        ...
        if action == "list":
            jobs = [job for job in await cron_mgr.list_jobs() if _extract_job_session(job) == current_umo]
            if not jobs:
