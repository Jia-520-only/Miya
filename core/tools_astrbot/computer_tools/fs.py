...
def _is_path_within_allowed_roots(path: str, umo: str) -> bool:
    ...
    return any(
        resolved == allowed_root or resolved.is_relative_to(allowed_root) for allowed_root in _read_allowed_roots(umo)
    )
    ...
    *,
    ...
    if restricted and not _is_path_within_allowed_roots(normalized_path, umo):
        ...
        raise PermissionError(
            f"Read access is restricted for this user. Allowed directories: {allowed}. Blocked path: {normalized_path}."
        )
    return normalized_path
    ...
    async def call(
        ...
        try:
            ...
            return await read_file_tool_result(
                ...
                offset=offset,
                limit=limit,
                workspace_dir=(str(_workspace_root(context.context.event.unified_msg_origin)) if local_env else None),
            )
    ...
    async def call(
        ...
        try:
            ...
            if not result.get("success", False):
                ...
                return f"Error writing file: {error_detail or 'unknown filesystem write error'}"
...
class FileEditTool(FunctionTool):
    ...
    async def call(
        ...
        try:
            ...
            if not result.get("success", False):
                error_detail = str(result.get("error", "") or "").strip()
                return f"Error editing file: {error_detail or 'unknown filesystem edit error'}"
            replacements = int(result.get("replacements", 0) or 0)
            mode_text = "all matches" if replace_all else "first match"
            return f"Edited {normalized_path}. Replaced {replacements} occurrence(s) using {mode_text} mode."
        except PermissionError as exc:
            return f"Error: {exc}"
        except Exception as exc:
...
class GrepTool(FunctionTool):
    ...
    def _normalize_search_paths(
        ...
        *,
        ...
        normalized = [_resolve_tool_path(path, local_env=local_env, umo=umo)] if path else []
        if not normalized:
            if restricted:
        ...
        if restricted:
            disallowed = [path for path in normalized if not _is_path_within_allowed_roots(path, umo)]
            if disallowed:
                allowed = ", ".join(_restricted_env_path_labels(umo))
        ...
        try:
            ...
            for search_path in search_paths:
                ...
                if not result.get("success", False):
                    ...
                    return f"Error searching files: {error_detail or 'unknown filesystem search error'}"
        ...
        try:
            ...
            if also_send_to_user:
                try:
                    ...
                    return f"File downloaded successfully to {local_path} but sending to user failed: {e}"
                ...
                return f"File downloaded successfully to {local_path} and sent to user as {sent_as}."
