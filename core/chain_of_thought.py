...
class ChainOfThought:
    ...
    async def start_chain(self, goal: str, context: Optional[Dict] = None) -> ThoughtChain:
        """
        开始新的思维链
        ...
        Returns:
            思维链对象
        """
        chain = ThoughtChain(id=self._generate_chain_id(), goal=goal, context=context or {})
    ...
    async def add_thought_step(
        ...
        if use_ai and self.ai_client:
            enhanced = await self._enhance_thought(self.current_chain, thought_type, content, reasoning)
    ...
    async def _enhance_thought(
        self, chain: ThoughtChain, thought_type: ThoughtType, content: str, reasoning: str
    ) -> Dict:
        """使用AI增强思考步骤"""
        try:
            prompt = f"""
...
"""
            ...
            response = await self.ai_client.chat_with_system_prompt(
                system_prompt="你是一个思考增强助手，帮助完善和丰富思考步骤。", user_message=prompt
            )
        ...
        return {"content": content, "reasoning": reasoning, "confidence": 0.5, "evidence": [], "alternatives": []}
    ...
    async def analyze(self, problem: str, context: Optional[Dict] = None) -> ThoughtChain:
        ...
        if not self.ai_client:
            ...
            await self.add_thought_step(
                ThoughtType.ANALYSIS, "分析问题", "AI客户端未配置，无法进行深度分析", confidence=0.3, use_ai=False
            )
        ...
        # 1. 问题分析
        await self.add_thought_step(
            ThoughtType.ANALYSIS, "分析问题的核心", "首先需要理解问题的本质和关键要素", confidence=0.7
        )

        # 2. 假设生成
        await self.add_thought_step(
            ThoughtType.HYPOTHESIS, "生成可能的解决方案", "基于问题分析，提出多个可能的解决方向", confidence=0.6
        )

        # 3. 推理验证
        await self.add_thought_step(
            ThoughtType.DEDUCTION, "验证假设的合理性", "通过逻辑推理验证每个假设的可行性", confidence=0.5
        )

        # 4. 反思和改进
        await self.add_thought_step(
            ThoughtType.REFLECTION, "反思推理过程", "检查是否有遗漏或错误的推理", confidence=0.4
        )

        # 5. 做出决策
        await self.add_thought_step(
            ThoughtType.DECISION, "确定最佳方案", "基于分析、推理和反思，选择最佳解决方案", confidence=0.8
        )
    ...
    async def reflect_on_chain(self, chain: Optional[ThoughtChain] = None) -> Dict:
        ...
        # 计算平均置信度
        if chain.steps:
            reflection["avg_confidence"] = sum(s.confidence for s in chain.steps) / len(chain.steps)

        # 统计思考类型分布
        for step in chain.steps:
            ttype = step.thought_type.value
            reflection["type_distribution"][ttype] = reflection["type_distribution"].get(ttype, 0) + 1

        # 生成改进建议
        if reflection["avg_confidence"] < 0.5:
            reflection["suggested_improvements"].append("平均置信度较低，建议增加更多的证据支持")

        if reflection["total_steps"] < 3:
            reflection["suggested_improvements"].append("思考步骤过少，建议进行更深入的分析")

        if ThoughtType.REFLECTION.value not in reflection["type_distribution"]:
            reflection["suggested_improvements"].append("缺少反思步骤，建议添加自我反思环节")
    ...
    def get_chain_summary(self, chain: Optional[ThoughtChain] = None) -> str:
        ...
        for i, step in enumerate(chain.steps, 1):
            ...
            lines.append(
                f"{i}. {icon} {step.thought_type.value} - {step.content[:80]}... (置信度: {step.confidence:.2f})"
            )
    ...
    def load_chain(self, path: str) -> Optional[ThoughtChain]:
        ...
        try:
            ...
            chain = ThoughtChain(id=data["id"], goal=data["goal"], context=data["context"])
