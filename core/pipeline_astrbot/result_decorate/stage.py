...
class ResultDecorateStage(Stage):
    async def initialize(self, ctx: PipelineContext) -> None:
        ...
        self.words_count_threshold = int(
            ctx.astrbot_config["platform_settings"]["segmented_reply"]["words_count_threshold"],
        )
        ...
        self.split_mode = ctx.astrbot_config["platform_settings"]["segmented_reply"].get("split_mode", "regex")
        self.regex = ctx.astrbot_config["platform_settings"]["segmented_reply"]["regex"]
        self.split_words = ctx.astrbot_config["platform_settings"]["segmented_reply"].get(
            "split_words", ["。", "？", "！", "~", "…"]
        )
        if self.split_words:
            escaped_words = sorted([re.escape(word) for word in self.split_words], key=len, reverse=True)
            self.split_words_pattern = re.compile(f"(.*?({'|'.join(escaped_words)})|.+$)", re.DOTALL)
        else:
    ...
    async def process(
        ...
        if len(result.chain) > 0:
            ...
            if (
                ...
                and ((self.only_llm_result and result.is_model_result()) or not self.only_llm_result)
            ):
                new_chain = []
                for comp in result.chain:
                    if isinstance(comp, Plain):
            ...
            if not should_tts and self.show_reasoning and event.get_extra("_llm_reasoning_content"):
                ...
                for comp in result.chain:
                    if isinstance(comp, Plain) and len(comp.text) > 1:
                        try:
                            ...
                            dual_output = self.ctx.astrbot_config["provider_tts_settings"]["dual_output"]
