    ...
    async def process(
        ...
        if len(result.chain) > 0:
            ...
            result.chain = [
                ...
                if not (isinstance(comp, Comp.Plain) and (not comp.text or not comp.text.strip()))
            ]
            ...
            if self.is_seg_reply_required(event):
                ...
                if all(comp.type in {ComponentType.Reply, ComponentType.At} for comp in result.chain):
                    # may fix #2670
                    logger.warning(
