...
class WhitelistCheckStage(Stage):
    ...
    async def process(
        ...
        if self.wl_ignore_admin_on_group and (
            event.role == "admin" and event.get_message_type() == MessageType.GROUP_MESSAGE
        ):
            return
        if self.wl_ignore_admin_on_friend and (
            event.role == "admin" and event.get_message_type() == MessageType.FRIEND_MESSAGE
        ):
            return
        if event.unified_msg_origin not in self.whitelist and str(event.get_group_id()).strip() not in self.whitelist:
            if self.wl_log:
                logger.info(
