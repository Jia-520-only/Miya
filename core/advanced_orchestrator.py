...
class AdvancedOrchestrator:
    ...
    def __init__(self, ai_client, tool_executor: callable, storage_dir: Optional[str] = None):
        """
        初始化高级编排器
        ...
        self.explorer = AutonomousExplorer(ai_client=ai_client, tool_executor=tool_executor)
        self.executor = IntelligentExecutor(
            tool_executor=tool_executor, enable_rollback=True, enable_result_validation=True
        )
    ...
    async def process_complex_task(
        self, goal: str, context: Optional[Dict] = None, enable_exploration: bool = True, enable_cot: bool = True
    ) -> Dict:
        """
        处理复杂任务（核心入口）
        ...
        try:
            ...
            if enable_cot:
                ...
                result["steps"].append(
                    {"phase": "思维链分析", "success": cot_result["success"], "summary": cot_result["summary"]}
                )
            ...
            result["steps"].append(
                {"phase": "任务规划", "success": planning_result["success"], "summary": planning_result["summary"]}
            )
        ...
        logger.info(
            f"复杂任务处理完成: {'成功' if result['success'] else '失败'} (耗时 {result['execution_time']:.2f}秒)"
        )
    ...
    async def _analyze_with_cot(self, goal: str, context: Optional[Dict]) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "summary": f"思维链分析失败: {str(e)}"}
        ...
        try:
            ...
            if context and "skills_registry" in context:
                ...
                if hasattr(skills_registry, "get_all_tools"):
                    ...
                    tools = (
                        list(skills_registry.tools.values())
                        if isinstance(skills_registry.tools, dict)
                        else skills_registry.tools
                    )
            ...
            # 分解任务
            tasks = await self.task_planner.decompose_task(goal=goal, context=context, tools=tools)
            ...
            return {"success": False, "error": str(e), "summary": f"任务规划失败: {str(e)}"}
    ...
    async def _explore_if_needed(self, goal: str, context: Optional[Dict]) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "findings": [], "summary": f"探索失败: {str(e)}"}
    ...
    async def _execute_tasks(self) -> Dict:
        ...
        try:
            ...
            if not ready_tasks:
                return {"success": True, "results": {}, "summary": "没有需要执行的任务"}
            ...
            if task_list:
                ...
                for task_id, exec_result in execution_results.items():
                    if exec_result.success:
                        self.task_planner.mark_task_completed(task_id, exec_result.output or "完成")
                    else:
                        self.task_planner.mark_task_failed(task_id, exec_result.error or "执行失败")
            ...
            return {"success": False, "results": {}, "summary": f"任务执行失败: {str(e)}", "details": None}

    async def _reflect_on_execution(self, result: Dict) -> Dict:
        """反思执行过程"""
        reflection = {"successful_tasks": [], "failed_tasks": [], "learnings": [], "improvements": []}
        ...
        if reflection["successful_tasks"]:
            reflection["learnings"].append(f"成功完成了 {len(reflection['successful_tasks'])} 个任务")

        if reflection["failed_tasks"]:
            reflection["improvements"].append(f"需要改进 {len(reflection['failed_tasks'])} 个失败的任务")

        # 查找反思结果
        if "thought_chain" in result:
            reflection["cot_reflection"] = await self.chain_of_thought.reflect_on_chain()
            reflection["improvements"].extend(reflection["cot_reflection"].get("suggested_improvements", []))
