...
class ProblemScanner:
    ...
    async def scan_all(self, path: str = ".", scanner_names: Optional[List[str]] = None, **kwargs) -> List[Problem]:
        """
        运行所有扫描器
    ...
    async def scan_by_type(self, problem_type: ProblemType, path: str = ".", **kwargs) -> List[Problem]:
        """
        按问题类型扫描
    ...
    def prioritize_problems(self, problems: List[Problem], max_count: int = 50) -> List[Problem]:
        """
        问题优先级排序
        ...
        # 排序
        sorted_problems = sorted(problems, key=lambda p: get_score(p), reverse=True)
