...
class CacheManagerAdapter:
    ...
    def __init__(
        self, default_ttl: float = 3600.0, max_size: int = 1000, max_memory_mb: float = 100.0, enable_stats: bool = True
    ):
        """初始化适配器"""
...
class PromptCacheAdapter:
    ...
    def get(self, context: Dict[str, Any]) -> Optional[str]:
        ...
        # 从统一缓存获取（同步访问，避免 asyncio.run 在事件循环中失败）
        try:
    ...
    def set(self, context: Dict[str, Any], prompt: str) -> None:
        ...
        # 存入统一缓存（同步访问，避免 asyncio.run 在事件循环中失败）
        try:
            self._cache.set_sync(key, prompt, self.ttl_seconds)
    ...
    def decorator(func: Callable):
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            cache_key = cache._generate_key(
                f"{key_prefix}:{func.__name__}", args[1:] if args and hasattr(args[0], "__class__") else args, kwargs
            )
