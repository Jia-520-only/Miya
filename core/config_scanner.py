...
class ConfigScanner(BaseScanner):
    ...
    def _check_sensitive_in_dict(self, data: Any, file_path: str, prefix: str = "") -> List[Problem]:
        """
        递归检查字典中的敏感信息
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, (dict, list)):
                    problems.extend(self._check_sensitive_in_dict(value, file_path, current_key))

        elif isinstance(data, list):
            for i, item in enumerate(data):
                current_key = f"{prefix}[{i}]"
                if isinstance(item, (dict, list)):
                    problems.extend(self._check_sensitive_in_dict(item, file_path, current_key))
