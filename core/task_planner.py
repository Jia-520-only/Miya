...
class TaskPlanner:
    ...
    async def decompose_task(
        self, goal: str, context: Optional[Dict] = None, tools: Optional[List[Dict]] = None
    ) -> List[Task]:
        """
        使用AI将目标分解为子任务
        ...
        if not self.ai_client:
            # 如果没有AI客户端，创建一个单任务
            task = Task(id=self._generate_task_id(), name="主任务", description=goal, priority=0)
            return [task]
...
输出格式（JSON）：
{
  "tasks": [
    ...
    def _build_decomposition_prompt(self, goal: str, context: Optional[Dict], tools: Optional[List[Dict]]) -> str:
        """构建任务分解提示词"""
    ...
    def get_task_graph(self) -> Dict:
        ...
        for task in self.tasks.values():
            nodes.append({"id": task.id, "label": task.name, "status": task.status.value, "priority": task.priority})
    ...
    def save_to_file(self, path: Optional[str] = None) -> bool:
        ...
        try:
            data = {"tasks": [task.to_dict() for task in self.tasks.values()], "task_id_counter": self._task_id_counter}
