        ...
        if self.audio_urls:
            for audio_url in self.audio_urls:
                if audio_url.startswith("http"):
                    ...
                    temp_audio_path = temp_dir / f"provider_request_audio_{uuid.uuid4().hex}{suffix}"
                    try:
    ...
    def completion_text(self, value) -> None:
        if self.result_chain:
            self.result_chain.chain = [
                comp for comp in self.result_chain.chain if not isinstance(comp, Comp.Plain)
            ]  # 清空 Plain 组件
    ...
    def to_openai_tool_calls(self) -> list[dict]:
        ...
        for idx, tool_call_arg in enumerate(self.tools_call_args):
            ...
            if self.tools_call_extra_content.get(self.tools_call_ids[idx]):
                payload["extra_content"] = self.tools_call_extra_content[self.tools_call_ids[idx]]
            ret.append(payload)
        return ret
        ...
        for idx, tool_call_arg in enumerate(self.tools_call_args):
            ret.append(
                ToolCall(
                    ...
                    extra_content=self.tools_call_extra_content.get(self.tools_call_ids[idx]),
                ),
            )
        return ret
