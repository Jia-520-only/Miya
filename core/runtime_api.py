...
class RuntimeAPIServer:
    ...
    async def _json_response(self, data: Any, status: int = 200):
        """返回JSON响应"""
        return web.json_response(data, status=status)
    
    async def _error_response(self, message: str, status: int = 400):
        """返回错误响应"""
    ...
    async def handle_get_endpoint(self, request: web.Request):
        ...
        if not endpoint:
            return await self._error_response("交互端不存在", 404)
    ...
    async def handle_start_endpoint(self, request: web.Request):
        ...
        if endpoint.status == "running":
            return await self._error_response("交互端已在运行", 400)
    ...
    async def handle_stop_endpoint(self, request: web.Request):
        ...
        if endpoint.status != "running":
            return await self._error_response("交互端未在运行", 400)
    ...
    async def handle_cognitive_events(self, request: web.Request):
        ...
        if not self.cognitive_memory:
            return await self._error_response("认知记忆系统未初始化", 500)
    ...
    async def handle_list_agents(self, request: web.Request):
        ...
        if not self.skills_registry:
            return await self._error_response("Skills注册表未初始化", 500)
