...
class AutonomousExplorer:
    ...
    def __init__(
        self, ai_client=None, tool_executor: Optional[Callable] = None, max_steps: int = 50, max_retries: int = 3
    ):
        """
        初始化自主探索器
    ...
    async def explore(
        self, goal: str, context: Optional[Dict] = None, initial_knowledge: Optional[Dict] = None
    ) -> ExplorationPlan:
        """
        开始自主探索
        ...
        Returns:
            探索计划（包含所有步骤和发现）
        """
        plan = ExplorationPlan(goal=goal, context=context or {}, max_steps=self.max_steps)
  ...
  "confidence": 置信度（0.0-1.0）
    ...
    def _parse_decision(self, response: str) -> ExplorationStep:
        ...
        try:
            ...
            # 默认思考
            return ExplorationStep(
                action=ExplorationAction.THINK, description="分析当前状态", reasoning=f"解析失败，改为思考: {str(e)}"
            )
    ...
    def _heuristic_decide(self, plan: ExplorationPlan) -> ExplorationStep:
        ...
        # 默认：思考
        return ExplorationStep(
            action=ExplorationAction.THINK, description="分析当前信息", reasoning="需要更多信息才能继续", confidence=0.5
        )

    async def _execute_step(self, step: ExplorationStep, plan: ExplorationPlan) -> str:
        """执行探索步骤"""
    ...
    async def _analyze_result(self, step: ExplorationStep, plan: ExplorationPlan) -> None:
        """分析步骤结果，提取发现"""
        if not step.result:
            return
...
"""
        ...
        try:
            response = await self.ai_client.chat_with_system_prompt(
                system_prompt="你是探索目标的评估者。根据提供的发现，判断目标是否已达成。", user_message=prompt
            )
    ...
    def load_plan(self, path: str) -> Optional[ExplorationPlan]:
        ...
        try:
            ...
            plan = ExplorationPlan(goal=data["goal"], context=data["context"], max_steps=data["max_steps"])
