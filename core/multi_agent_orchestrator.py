    ...
    async def execute(self, task: Dict) -> Dict:
        ...
        try:
            ...
            self.execution_history.append(
                {"task_id": task.get("id"), "result": result, "timestamp": datetime.now().timestamp()}
            )
...
class MultiAgentOrchestrator:
    ...
    async def _execute_parallel(self, assignments: Dict[str, str], subtasks: List[SubTask]) -> Dict[str, Any]:
        ...
        async def execute_single(subtask_id: str, agent_id: str):
            ...
            task_data = {"id": subtask.task_id, "description": subtask.description, "input_data": subtask.input_data}
        ...
        # 并行执行所有子任务
        tasks = [execute_single(subtask_id, agent_id) for subtask_id, agent_id in assignments.items()]
        ...
        for subtask_id, subtask in results.items():
            if subtask.status == TaskStatus.COMPLETED:
                successful_results.append(
                    {"task_id": subtask_id, "agent_id": subtask.assigned_agent, "result": subtask.result}
                )
            else:
                failed_tasks.append({"task_id": subtask_id, "error": subtask.error})
    ...
    async def send_message(self, sender_id: str, receiver_id: str, content: Any, message_type: str = "text") -> bool:
        """发送消息"""
    ...
    async def receive_message(self, agent_id: str, timeout: float = 5.0) -> Optional[AgentMessage]:
        """接收消息"""
        try:
            return await asyncio.wait_for(self._message_handlers[agent_id].get(), timeout=timeout)
        except asyncio.TimeoutError:
            return None

    async def broadcast_message(self, sender_id: str, content: Any, message_type: str = "text") -> int:
        """广播消息给所有Agent"""
        count = 0
    ...
    async def get_task_status(self, task_id: str) -> Optional[Dict]:
        """获取任务状态"""
        subtasks = [st for st in self.tasks.values() if st.task_id.startswith(task_id) or st.task_id == task_id]
        if not subtasks:
            return None
