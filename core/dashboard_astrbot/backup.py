...
class BackupRoute(Route):
    ...
    def _make_progress_callback(self, task_id: str):
        ...
        async def _callback(stage: str, current: int, total: int, message: str = "") -> None:
    ...
    async def upload_init(self):
        ...
        try:
            ...
            logger.info(
                f"初始化分片上传: upload_id={upload_id}, filename={unique_filename}, total_chunks={total_chunks}"
            )
    ...
    async def upload_complete(self):
        ...
        try:
            ...
            if len(received) != total:
                ...
                return Response().error(f"分片不完整，缺少: {sorted(missing)[:10]}...").__dict__
    ...
    async def import_backup(self):
        ...
        try:
            ...
            if not confirmed:
                return Response().error("请先确认导入。导入将会清空并覆盖现有数据，此操作不可撤销。").__dict__
