...
class KnowledgeBaseRoute(Route):
    ...
    def _set_task_result(self, task_id: str, status: str, result: Any = None, error: str | None = None) -> None:
        self.upload_tasks[task_id] = {
            "status": status,
            "result": result,
            "error": error,
        ...
        try:
            ...
            for file_idx, file_info in enumerate(files_to_upload):
                try:
                    ...
                    # 创建进度回调函数
                    progress_callback = self._make_progress_callback(task_id, file_idx, file_info["file_name"])
                    ...
                    failed_docs.append(
                        {
                            "file_name": file_info["file_name"],
                            "error": self._format_failed_doc_error(file_info["file_name"], e),
                        },
                    )
    ...
    async def list_kbs(self):
        ...
        try:
            ...
            return Response().ok({"items": kb_list, "page": page, "page_size": page_size}).__dict__
    ...
    async def create_kb(self):
        ...
        try:
            ...
            if not prv or not isinstance(prv, EmbeddingProvider):
                return Response().error(f"嵌入模型不存在或类型错误({type(prv)})").__dict__
            try:
            ...
            if rerank_provider_id:
                rerank_prv: RerankProvider = await kb_manager.provider_manager.get_provider_by_id(
                    rerank_provider_id,
                )  # type: ignore
                if not rerank_prv:
                ...
                try:
                    ...
                    return Response().error(f"测试重排序模型失败: {e!s}，请检查平台日志输出。").__dict__
    ...
    async def list_documents(self):
        ...
        try:
            ...
            return Response().ok({"items": doc_list, "page": page, "page_size": page_size}).__dict__
    ...
    async def upload_document(self):
        ...
        try:
            ...
            if content_type and "multipart/form-data" not in content_type:
                return Response().error("Content-Type 须为 multipart/form-data").__dict__
    ...
    async def import_documents(self):
        ...
        try:
            ...
            kb_id, documents, batch_size, tasks_limit, max_retries = self._validate_import_request(data)
