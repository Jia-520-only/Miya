    ...
    async def create_job(self):
        try:
            ...
            if run_once and not run_at:
                return jsonify(Response().error("run_at is required when run_once=true").__dict__)
            if (not run_once) and not cron_expression:
                return jsonify(Response().error("cron_expression is required when run_once=false").__dict__)
            ...
            if run_at:
                try:
                    ...
                    return jsonify(Response().error("run_at must be ISO datetime").__dict__)
        ...
        try:
            ...
            if job.job_type == "active_agent":
                ...
                if "session" in payload:
                    ...
                    if not session:
                        return jsonify(Response().error("session cannot be empty").__dict__)
                ...
                if "note" in payload:
                    ...
                    if not note:
                        return jsonify(Response().error("note cannot be empty").__dict__)
                    ...
                    if not description:
                        return jsonify(Response().error("description cannot be empty").__dict__)
                ...
                next_cron_expression = (
                    payload.get("cron_expression") if "cron_expression" in payload else job.cron_expression
                )
                ...
                if "cron_expression" in payload:
                    ...
                    if not cron_expression:
                        return jsonify(Response().error("cron_expression cannot be empty").__dict__)
