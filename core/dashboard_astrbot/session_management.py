...
class SessionManagementRoute(Route):
    ...
    async def _get_umo_rules(self, page: int = 1, page_size: int = 10, search: str = "") -> tuple[dict, int]:
    ...
    async def list_session_rule(self):
        ...
        try:
            ...
            umo_rules, total = await self._get_umo_rules(page=page, page_size=page_size, search=search)
            ...
            available_personas = [{"name": p["name"], "prompt": p.get("prompt", "")} for p in persona_mgr.personas_v3]
    ...
    async def update_session_rule(self):
        ...
        try:
            ...
            return Response().ok({"message": f"规则 {rule_key} 已更新", "umo": umo}).__dict__
        except Exception as e:
    ...
    async def delete_session_rule(self):
        ...
        try:
            ...
            if rule_key:
                ...
                return Response().ok({"message": f"规则 {rule_key} 已删除", "umo": umo}).__dict__
            else:
                # 删除该 umo 的所有规则
