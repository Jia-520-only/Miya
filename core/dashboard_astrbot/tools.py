        ...
        try:
            ...
            for key, value in server_data.items():
                if key not in ["name", "active", "tools", "errlogs"]:  # 排除特殊字段
                    if key == "mcpServers":
                        try:
                            server_config = _extract_mcp_server_config(server_data["mcpServers"])
            ...
            if not has_valid_config:
                return Response().error("A valid server configuration is required").__dict__
            ...
            if self.tool_mgr.save_mcp_config(config):
                try:
                    ...
                    if not rollback_ok:
                        err_msg += " Configuration rollback failed. Please check the config manually."
                    return Response().error(err_msg).__dict__
                return Response().ok(None, f"Successfully added MCP server {name}").__dict__
    ...
    async def update_mcp_server(self):
        try:
            ...
            if self.tool_mgr.save_mcp_config(config):
                # 处理MCP客户端状态变化
                if active:
                    if old_name in self.tool_mgr.mcp_server_runtime_view or not only_update_active or is_rename:
                        try:
                            await self.tool_mgr.disable_mcp_server(old_name, timeout=10)
                        except TimeoutError as e:
                            return (
                                Response()
                                .error(f"Timed out while disabling MCP server {old_name} before enabling: {e!s}")
                                .__dict__
                            )
                        except Exception as e:
                            logger.error(traceback.format_exc())
                            return (
                                Response()
                                .error(f"Failed to disable MCP server {old_name} before enabling: {e!s}")
                                .__dict__
                            )
                    try:
                        await self.tool_mgr.enable_mcp_server(
                        ...
                        return Response().error(f"Timed out while enabling MCP server {name}.").__dict__
                    except Exception as e:
                        logger.error(traceback.format_exc())
                        return Response().error(f"Failed to enable MCP server {name}: {e!s}").__dict__
                # 如果要停用服务器
                elif old_name in self.tool_mgr.mcp_server_runtime_view:
                    try:
                        await self.tool_mgr.disable_mcp_server(old_name, timeout=10)
                    except TimeoutError:
                        return Response().error(f"Timed out while disabling MCP server {old_name}.").__dict__
                    except Exception as e:
                        logger.error(traceback.format_exc())
                        return Response().error(f"Failed to disable MCP server {old_name}: {e!s}").__dict__

                return Response().ok(None, f"Successfully updated MCP server {name}").__dict__
            return Response().error("Failed to save configuration").__dict__
        except Exception as e:
            logger.error(traceback.format_exc())
            ...
            if self.tool_mgr.save_mcp_config(config):
                if name in self.tool_mgr.mcp_server_runtime_view:
                    try:
                        ...
                        return Response().error(f"Timed out while disabling MCP server {name}.").__dict__
                    except Exception as e:
                        logger.error(traceback.format_exc())
                        return Response().error(f"Failed to disable MCP server {name}: {e!s}").__dict__
                return Response().ok(None, f"Successfully deleted MCP server {name}").__dict__
            return Response().error("Failed to save configuration").__dict__
        except Exception as e:
            logger.error(traceback.format_exc())
        ...
        try:
            ...
            if "mcpServers" in config:
                ...
                if isinstance(mcp_servers, dict) and len(mcp_servers) > 1:
                    return Response().error("Only one MCP server configuration can be tested at a time").__dict__
                try:
                    config = _extract_mcp_server_config(mcp_servers)
                except EmptyMcpServersError:
                    return Response().error("MCP server configuration cannot be empty").__dict__
                except ValueError as e:
                    return Response().error(f"{e!s}").__dict__
            elif not config:
                return Response().error("MCP server configuration cannot be empty").__dict__
            ...
            return Response().ok(data=tools_name, message="🎉 MCP server is available!").__dict__
        ...
        try:
            ...
            for tool in tools:
                ...
                if self.tool_mgr.is_builtin_tool(tool.name):
                    ...
                    builtin_config_tags = [status for status in builtin_config_statuses if status["enabled"]]
    ...
    async def toggle_tool(self):
        ...
        try:
            ...
            if not tool_name or action is None:
                return Response().error("Missing required parameters: name or activate").__dict__

            if self.tool_mgr.is_builtin_tool(tool_name):
                return Response().error("Builtin tools are read-only and cannot be toggled.").__dict__
            ...
            return Response().error(f"Tool {tool_name} does not exist or the operation failed.").__dict__
    ...
    async def sync_provider(self):
        ...
        try:
            ...
            match provider_name:
                ...
                case _:
                    return Response().error(f"Unknown provider: {provider_name}").__dict__
