    ...
    async def create_persona(self):
        ...
        try:
            ...
            if begin_dialogs and len(begin_dialogs) % 2 != 0:
                return Response().error("预设对话数量必须为偶数（用户和助手轮流对话）").__dict__
    ...
    async def update_persona(self):
        ...
        try:
            ...
            if has_custom_error_message:
                if custom_error_message is not None and not isinstance(custom_error_message, str):
                    return Response().error("自定义报错回复信息必须是字符串").__dict__
        ...
        try:
            ...
            for item in items:
                if not all(k in item for k in ("id", "type", "sort_order")):
                    return Response().error("每个 item 必须包含 id, type, sort_order 字段").__dict__
                if item["type"] not in ("persona", "folder"):
                    return Response().error("type 字段必须是 'persona' 或 'folder'").__dict__
