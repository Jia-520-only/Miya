...
class StatRoute(Route):
    ...
    def is_default_cred(self):
        ...
        return username == "astrbot" and password == "77b90590a8945a7d36c963981a307dc9" and not DEMO_MODE
    ...
    async def get_storage_status(self):
        try:
            ...
            return Response().error("获取存储占用失败，请查看后端日志了解详情。").__dict__
    ...
    async def test_ghproxy_connection(self):
        ...
        try:
            ...
            async with (
                ...
                return Response().error(f"Failed. Status code: {response.status}").__dict__
        except Exception as e:
            logger.error(traceback.format_exc())
            return Response().error(f"Error: {e!s}").__dict__
