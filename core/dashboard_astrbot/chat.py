...
class ChatRoute(Route):
    ...
    async def get_file(self):
        ...
        try:
            ...
            if not os.path.exists(real_file_path):
                # try legacy
                file_path = os.path.join(self.legacy_img_dir, os.path.basename(filename))
    ...
    async def _create_attachment_from_file(self, filename: str, attach_type: str) -> dict | None:
    ...
    def _extract_web_search_refs(self, accumulated_text: str, accumulated_parts: list) -> dict:
    ...
    def _build_webchat_unified_msg_origin(self, session) -> str:
        ...
        return f"{session.platform_id}:{message_type}:{session.platform_id}!{session.creator}!{session.session_id}"
    ...
    async def _delete_threads_by_ids(self, thread_ids: list[str], creator: str) -> None:
        for thread_id in thread_ids:
            unified_msg_origin = self._build_thread_unified_msg_origin(creator, thread_id)
    ...
    def _find_checkpoint_index(self, history: list[dict], checkpoint_id: str) -> int | None:
        for index, message in enumerate(history):
            if get_checkpoint_id(message) == checkpoint_id:
                return index
        return None

    def _find_turn_range(self, history: list[dict], checkpoint_id: str) -> tuple[int, int] | None:
    ...
    def _find_turn_user_index(self, history: list[dict], start: int, end: int) -> int | None:
    ...
    def _find_turn_final_assistant_index(self, history: list[dict], start: int, end: int) -> int | None:
    ...
    async def _delete_platform_history_after(self, session, message_id: int) -> list[int]:
    ...
    async def chat(self, post_data: dict | None = None):
        ...
        if "session_id" not in post_data and "conversation_id" not in post_data:
            return Response().error("Missing key: session_id or conversation_id").__dict__
        ...
        if not webchat_message_parts_have_content(message_parts):
            return Response().error("Message content is empty (reply only is not allowed)").__dict__
        ...
        async def stream():
            ...
            try:
                ...
                async with track_conversation(self.running_convs, webchat_conv_id):
                    while True:
                        result, should_break = await _poll_webchat_stream_result(back_queue, username)
                        if should_break:
                        ...
                        try:
                            ...
                            if not client_disconnected:
                                logger.debug(f"[WebChat] 用户 {username} 断开聊天长连接。 {e}")
                            client_disconnected = True
                        ...
                        should_save = False
                        if msg_type == "end":
                            should_save = message_accumulator.has_content() or bool(refs or agent_stats)
    ...
    async def create_thread(self):
        ...
        if not checkpoint_id:
            return Response().error("Parent message is not linked to LLM history").__dict__
        ...
        conversation_id, history = await self._load_current_conversation_history(session)
    ...
    async def update_message(self):
        ...
        if not latest_user_record or latest_user_record.id != message_id:
            return Response().error("Only the latest user message can be edited").__dict__

        checkpoint_id = record.llm_checkpoint_id
        if not checkpoint_id:
            return Response().error("This message is not linked to LLM history and cannot be edited").__dict__
        ...
        deleted_message_ids = await self._delete_platform_history_after(session, message_id)
    ...
    async def regenerate_message(self):
        ...
        if not conversation_id or not turn_range:
            return Response().error("Linked checkpoint not found").__dict__
        if not self._is_latest_checkpoint(history, checkpoint_id):
            return Response().error("Regenerating older turns requires branching").__dict__
