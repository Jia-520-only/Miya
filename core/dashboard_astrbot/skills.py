...
class SkillsRoute(Route):
    ...
    async def _delete_neo_candidate(self, client: Any, candidate_id: str, reason: str | None):
    ...
    async def get_skills(self):
        try:
            ...
            skills = skill_mgr.list_skills(active_only=False, runtime=runtime, show_sandbox_path=False)
            return (
                Response()
                .ok(
                    {
        ...
        try:
            ...
            try:
                try:
                    ...
                    skill_name = skill_mgr.install_skill_from_zip(temp_path, overwrite=False)
            except Exception:
            ...
            return Response().ok({"name": skill_name}, "Skill uploaded successfully.").__dict__
        except Exception as e:
            logger.error(traceback.format_exc())
            return Response().error(str(e)).__dict__
        finally:
    ...
    async def download_skill(self):
        try:
            ...
            if skill_mgr.is_sandbox_only_skill(name):
                return Response().error("Sandbox preset skill cannot be downloaded from local skill files.").__dict__
    ...
    async def promote_neo_candidate(self):
        ...
        async def _do(client):
            ...
            if did_sync_to_local:
                logger.info(f"[Neo] Stable release synced to local: skill={sync_json.get('local_skill_name', '')}")

            if result.get("sync_error"):
                resp = Response().error(
                    f"Stable promote synced failed and has been rolled back. sync_error={result['sync_error']}"
                )
                resp.data = {
    ...
    async def sync_neo_release(self):
        ...
        async def _do(client):
            ...
            logger.info(
                f"[Neo] Release synced to local: skill={result.local_skill_name}, release_id={result.release_id}"
            )
            return (
                Response()
                .ok(
