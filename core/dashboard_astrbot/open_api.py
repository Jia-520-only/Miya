...
class OpenApiRoute(Route):
    ...
    async def chat_send(self):
        ...
        effective_username, username_err = self._resolve_open_username(post_data.get("username"))
        if username_err:
        ...
        if config_id:
            ...
            try:
                ...
                return Response().error(f"Failed to update chat config route: {e}").__dict__
        try:
    ...
    async def _handle_chat_ws_send(self, post_data: dict) -> None:
        ...
        if username_err or not effective_username:
            await self._send_chat_ws_error(username_err or "Invalid username", "BAD_USER")
            return
        ...
        try:
            ...
            while True:
                ...
                if msg_type == "plain":
                    ...
                    part = await self.chat_route._create_attachment_from_file(filename, "image")
                    message_accumulator.add_attachment(part)
                elif msg_type == "record":
                    filename = str(result_text).replace("[RECORD]", "")
                    part = await self.chat_route._create_attachment_from_file(filename, "record")
                    message_accumulator.add_attachment(part)
                elif msg_type == "file":
                    filename = str(result_text).replace("[FILE]", "")
                    part = await self.chat_route._create_attachment_from_file(filename, "file")
                    message_accumulator.add_attachment(part)
                elif msg_type == "video":
                    filename = str(result_text).replace("[VIDEO]", "")
                    part = await self.chat_route._create_attachment_from_file(filename, "video")
                    message_accumulator.add_attachment(part)
            ...
            await self._send_chat_ws_error(f"Failed to process message: {e}", "PROCESSING_ERROR")
        finally:
    ...
    async def get_chat_sessions(self):
        username, username_err = self._resolve_open_username(request.args.get("username"))
        if username_err:
            return Response().error(username_err).__dict__
    ...
    async def send_message(self):
        ...
        if not platform_inst:
            return Response().error(f"Bot not found or not running for platform: {platform_id}").__dict__

        try:
            message_chain = await self._build_message_chain_from_payload(message_payload)
