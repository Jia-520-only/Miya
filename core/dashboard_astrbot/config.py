...
def save_config(post_config: dict, config: AstrBotConfig, is_core: bool = False) -> None:
    ...
    try:
        if is_core:
            ...
            errors, post_config = validate_config(post_config, getattr(config, "schema", {}), is_core)
...
class ConfigRoute(Route):
    ...
    async def delete_provider_source(self):
        ...
        target_idx = next(
            (i for i, ps in enumerate(provider_sources) if ps.get("id") == provider_source_id),
            -1,
        )
        ...
        await self.core_lifecycle.provider_manager.delete_provider(provider_source_id=provider_source_id)
    ...
    async def update_provider_source(self):
        ...
        if reload_errors:
            return Response().error("更新成功，但部分提供商重载失败: " + ", ".join(reload_errors)).__dict__
    ...
    async def get_provider_config_list(self):
        ...
        for provider in ps:
            ...
            if ps_id and ps_id in p_source_pt and p_source_pt[ps_id] in provider_type_ls:
                # chat
                prov = self.core_lifecycle.provider_manager.get_merged_provider_config(provider)
    ...
    async def get_provider_model_list(self):
        ...
        if not isinstance(provider, Provider):
            return Response().error(f"提供商 {provider_id} 类型不支持获取模型列表").__dict__
        ...
        try:
            ...
            if provider_type not in provider_cls_map:
                try:
                    ...
                    logger.error(traceback.format_exc())
                    return Response().error("提供商适配器加载失败，请检查提供商类型配置或查看服务端日志").__dict__

            # 获取对应的 provider 类
            if provider_type not in provider_cls_map:
                return Response().error(f"未找到适用于 {provider_type} 的提供商适配器").__dict__
    ...
    async def get_provider_source_models(self):
        ...
        try:
            ...
            if not provider_source:
                return Response().error(f"未找到 ID 为 {provider_source_id} 的 provider_source").__dict__
            ...
            if not issubclass(cls_type, Provider):
                return Response().error(f"提供商 {provider_type} 不支持获取模型列表").__dict__
            ...
            return Response().ok({"models": models, "model_metadata": metadata_map}).__dict__
        except Exception as e:
            logger.error(traceback.format_exc())
    ...
    async def post_plugin_configs(self):
        ...
        try:
            ...
            return Response().ok(None, f"保存插件 {plugin_name} 成功~ 机器人正在热重载插件。").__dict__
        except Exception as e:
            return Response().error(str(e)).__dict__
    ...
    async def post_update_provider(self):
        ...
        try:
            await self.core_lifecycle.provider_manager.update_provider(origin_provider_id, new_config)
        except Exception as e:
            return Response().error(str(e)).__dict__
    ...
    async def post_delete_provider(self):
        ...
        try:
            await self.core_lifecycle.provider_manager.delete_provider(provider_id=provider_id)
        except Exception as e:
            return Response().error(str(e)).__dict__
    ...
    def _inject_platform_metadata_with_i18n(self, platform, metadata, platform_i18n_translations: dict):
        ...
        for platform in platform_registry:
            if platform.default_config_tmpl:
                ...
                if platform.config_metadata:
                    self._inject_platform_metadata_with_i18n(platform, metadata, platform_i18n_translations)
    ...
    async def _save_astrbot_configs(self, post_configs: dict, conf_id: str | None = None) -> None:
        try:
    ...
    async def _save_plugin_configs(self, post_configs: dict, plugin_name: str) -> None:
        ...
        try:
            errors, post_configs = validate_config(post_configs, getattr(md.config, "schema", {}), is_core=False)
            if errors:
                raise ValueError(f"格式校验未通过: {errors}")
