...
class PluginRoute(Route):
    ...
    async def install_plugin_upload(self):
        ...
        try:
            ...
            ignore_version_check = str(form_data.get("ignore_version_check", "false")).lower() == "true"
    ...
    async def update_all_plugins(self):
        ...
        for name, result in zip(plugin_names, raw_results, strict=False):
            ...
            if isinstance(result, BaseException):
                results.append({"name": name, "status": "error", "message": str(result)})
            else:
        ...
        for name in changelog_names:
            ...
            if os.path.isfile(changelog_path):
                try:
                    ...
                    return Response().ok({"content": changelog_content}, "成功获取更新日志").__dict__
                except Exception as e:
