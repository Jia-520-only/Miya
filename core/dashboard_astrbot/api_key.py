...
from core.astrbot_compat.utils import normalize_datetime_utc
...
class ApiKeyRoute(Route):
    ...
    async def list_api_keys(self):
        ...
        return Response().ok(data=[self._serialize_api_key(key) for key in keys]).__dict__
    ...
    async def create_api_key(self):
        ...
        if scopes is None:
            ...
            normalized_scopes = [scope for scope in scopes if isinstance(scope, str) and scope in ALL_OPEN_API_SCOPES]
        ...
        if expires_in_days is not None:
            ...
            if expires_in_days_int <= 0:
                return Response().error("expires_in_days must be greater than 0").__dict__
            expires_at = datetime.now(UTC) + timedelta(days=expires_in_days_int)
