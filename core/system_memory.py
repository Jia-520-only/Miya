...
@dataclass
class SystemMemoryItem:
    """系统记忆项（与 memory/core.py 的 MemoryItem 不同，此用于系统级配置/修复记忆）"""
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> "SystemMemoryItem":
        """从字典创建"""
        data = data.copy()
...
class SystemMemory:
    ...
    def __init__(self, memory_dir: str = ".memory"):
        ...
        # 记忆存储
        self.memories: Dict[str, SystemMemoryItem] = {}

        # 按类型分类
        self.by_type: Dict[MemoryType, List[SystemMemoryItem]] = defaultdict(list)

        # 按键索引
        self.by_key: Dict[str, SystemMemoryItem] = {}
    ...
    def recall(self, key_parts: List[str], type: Optional[MemoryType] = None) -> Optional[SystemMemoryItem]:
        """
        回忆一个信息
    ...
    def recall_by_type(self, type: MemoryType) -> List[SystemMemoryItem]:
        """
        按类型回忆所有记忆
    ...
    def get_report(self) -> str:
        ...
        if recent_fixes:
            for fix in recent_fixes:
                ...
                lines.append(f"  {status} {fix.problem_type}/{fix.severity}: {fix.file_path or 'N/A'}")
        else:
            lines.append("  暂无")
    ...
    def load(self, file_path: Optional[str] = None):
        ...
        try:
            ...
            for m_data in data.get("memories", []):
                memory = SystemMemoryItem.from_dict(m_data)
