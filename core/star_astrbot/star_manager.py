...
class PluginManager:
    ...
    def _try_import_from_installed_dependencies(
        ...
        try:
            logger.info(f"插件 {root_dir_name} 导入失败，尝试从已安装依赖恢复: {import_exc!s}")
            ...
            logger.info(f"插件 {root_dir_name} 已安装依赖恢复失败，将重新安装依赖: {recover_exc!s}")
            return None
        ...
        *,
        ...
        if recovery_state.mode is ImportDependencyRecoveryMode.PRELOAD_AND_RECOVER:
            try:
                ...
                logger.info(f"插件 {root_dir_name} 预加载已安装依赖失败，将继续常规导入: {preload_exc!s}")
    ...
    def _validate_importable_name(plugin_name: str) -> None:
        if "/" in plugin_name or "\\" in plugin_name:
            raise ValueError("metadata.yaml 中 name 含有路径分隔符，不可用于 importlib 加载。")
        if not plugin_name.isidentifier() or keyword.iskeyword(plugin_name):
            raise Exception("metadata.yaml 中 name 不是合法的模块名称（应为合法 Python 标识符且非关键字）。")
    ...
    def _get_plugin_related_modules(
        ...
        return [key for key in list(sys.modules.keys()) if key.startswith(f"{prefix}{plugin_root_dir}")]
                    ...
                    if not ignore_version_check:
                        ...
                        if not is_valid:
                            raise PluginVersionIncompatibleError(
                                error_message or "The plugin is not compatible with the current AstrBot version."
                            )
    ...
    async def install_plugin(self, repo_url: str, proxy: str = "", ignore_version_check: bool = False):
        """从仓库 URL 安装插件
    ...
    async def install_plugin_from_file(self, zip_file_path: str, ignore_version_check: bool = False):
