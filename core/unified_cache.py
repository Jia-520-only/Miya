...
class BaseCacheLayer(ABC):
    ...
    def get_sync(self, key: str) -> Optional[Any]:
        """同步获取缓存值（供同步上下文调用，避免 asyncio.run 在事件循环中失败）"""
        return self._get_sync(self._generate_key(key))

    def set_sync(self, key: str, value: Any, ttl: Optional[float] = None) -> bool:
        """同步设置缓存值（供同步上下文调用）"""
        return self._set_sync(self._generate_key(key), value, ttl)

    def delete_sync(self, key: str) -> bool:
        """同步删除缓存值"""
        return self._delete_sync(self._generate_key(key))
