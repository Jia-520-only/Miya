...
class WebAPI:
    ...
    def _setup_routes(self):
        ...
        @self.router.get("/cross-terminal/messages")
        async def get_cross_terminal_messages(
            ...
            """获取跨端消息（桌面端轮询）"""
            try:
                from webnet.CrossTerminalNet.hub import get_cross_terminal_hub, TERMINAL_TYPE_DESKTOP
                ...
                logger.error(f"[WebAPI] 获取跨端消息失败: {e}")
                return {
                    "success": False,
                    "messages": [],
                    "error": str(e)
        ...
        @self.router.post("/cross-terminal/send")
        async def send_cross_terminal_message(
            ...
            try:
                ...
                from webnet.CrossTerminalNet.hub import TerminalType
                ...
                logger.error(f"[WebAPI] 发送跨端消息失败: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.router.get("/cross-terminal/devices")
        async def get_online_devices():
            ...
            try:
                ...
                logger.error(f"[WebAPI] 获取设备列表失败: {e}")
                return {
                    "success": False,
                    "devices": [],
                    "error": str(e)
        ...
        @self.router.post("/cross-terminal/sync-state")
        async def sync_state(key: str, value: str):
            ...
            try:
                ...
                logger.error(f"[WebAPI] 状态同步失败: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }
        ...
        async def tts_speak(request: Dict[str, Any]):
            ...
            try:
                ...
                # 调用TTS生成
                audio_data = await tts_net.synthesize(text, engine=engine)
        ...
        async def tts_engines():
            ...
            try:
                ...
                logger.error(f"[WebAPI] TTS引擎列表获取失败: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.router.get("/tts/status")
        async def tts_status():
            ...
            try:
                ...
                engines_status = {}
                for name, engine in tts_net.registry.engines.items():
                    engines_status[name] = {
                        "initialized": getattr(engine, 'is_initialized', False),
                        "available": getattr(engine, 'is_initialized', False)
                    }

                return {
                    "success": True,
                    "initialized": True,
                    "current_engine": tts_net.registry.current_engine,
                    "default_engine": tts_net.registry.default_engine,
                    "engines": engines_status
                }
            except Exception as e:
                logger.error(f"[WebAPI] TTS状态获取失败: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }
