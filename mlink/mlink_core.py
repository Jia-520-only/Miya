...
class MLinkCore:
    ...
    def __init__(self, enable_queue: bool = True, enable_monitor: bool = True):
        ...
        # 流量统计
        self.flow_stats = {flow_type: {"sent": 0, "received": 0} for flow_type in self.FLOW_TYPES}
    ...
    async def send(self, message: Message, available_nodes: List[str]) -> bool:
        ...
        if self.flow_monitor:
            await self.flow_monitor.trace_message(message, "sent", {"destination": available_nodes})
        ...
        if trust < 0.3:
            ...
            if self.flow_monitor:
                await self.flow_monitor.trace_message(message, "failed", {"reason": "low_trust", "trust_value": trust})
        ...
        if self.flow_monitor:
            await self.flow_monitor.record_flow(
                message.flow_type, message_size=len(str(message.content)), is_error=False
            )
            await self.flow_monitor.update_node_stats(message.source, "sent")
    ...
    async def receive(self, message: Message) -> None:
        ...
        # 追踪接收
        if self.flow_monitor:
            await self.flow_monitor.trace_message(message, "received", {"destination": message.destination})
            await self.flow_monitor.update_node_stats(message.destination or "unknown", "received")

    def register_node(self, node_id: str, capabilities: List[str], load: float = 0.0) -> None:
        """注册节点"""
        self.router.update_node_status(node_id, {"available": True, "capabilities": capabilities, "load": load})
