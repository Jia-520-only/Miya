...
class TrustTransmit:
    ...
    def propagate_trust(self, path: List[str], initial_trust: float = 1.0) -> float:
        ...
        for i in range(len(path) - 1):
            ...
            self.propagation_history.append(
                {"path": path[: i + 2], "trust": current_trust, "timestamp": datetime.now()}
            )
    ...
    def decay_trust(self, older_than_hours: int = 24) -> int:
        ...
        # 查找需要衰减的信任关系
        keys_to_update = [key for key, value in self.trust_matrix.items() if self._should_decay(key, cutoff)]
    ...
    def get_trust_stats(self) -> Dict:
        ...
        if not self.trust_matrix:
            return {"total_relations": 0, "avg_trust": 0.0}
