...
class ProviderGoogleGenAI(Provider):
    ...
    def _init_safety_settings(self) -> None:
        ...
        self.safety_settings = [
            ...
            if (threshold_str := user_safety_config.get(config_key)) and threshold_str in self.THRESHOLD_MAPPING
        ]
    ...
    async def _prepare_query_config(
        ...
        # 流式输出不支持图片模态
        if self.provider_settings.get("streaming_response", False) and "IMAGE" in modalities:
                ...
                if isinstance(content, str):
                    ...
                    for tool in message["tool_calls"]:
                        ...
                        if "extra_content" in tool and tool["extra_content"]:
                            ts_bs64 = tool["extra_content"].get("google", {}).get("thought_signature")
                            if ts_bs64:
    ...
    def _extract_reasoning_content(self, candidate: types.Candidate) -> str:
        ...
        thought_buf: list[str] = [(p.text or "") for p in candidate.content.parts if p.thought]
        return "".join(thought_buf).strip()

    def _extract_usage(self, usage_metadata: types.GenerateContentResponseUsageMetadata) -> TokenUsage:
    ...
    def _process_content_parts(
        ...
        *,
        ...
        for part in result_parts:
            ...
            if part.function_call and part.function_call.name is not None and part.function_call.args is not None:
                ...
                if part.thought_signature:
                    ...
                    llm_response.tools_call_extra_content[tool_call_id] = {"google": {"thought_signature": ts_bs64}}
        ...
        while True:
            try:
                ...
                if "Developer instruction is not enabled" in e.message:
                    ...
                    or "Model does not support the requested response modalities" in e.message
