...
class ProviderAnthropic(Provider):
    ...
    def _ensure_usable_response(
        ...
        *,
        ...
        raise EmptyModelOutputError(
            f"Anthropic completion has no usable output. completion_id={completion_id}, stop_reason={stop_reason}"
        )
    ...
    async def _query(self, payloads: dict, tools: ToolSet | None) -> LLMResponse:
        if tools and (tool_list := tools.get_func_desc_anthropic_style()):
            ...
            payloads["tool_choice"] = {"type": "any" if payloads.get("tool_choice") == "required" else "auto"}
        ...
        try:
            completion = await self.client.messages.create(**payloads, stream=False, extra_body=extra_body)
