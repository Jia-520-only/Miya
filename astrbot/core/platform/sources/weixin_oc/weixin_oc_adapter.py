...
class WeixinOCAdapter(Platform):
    ...
    def __init__(
        ...
        self.base_url = str(platform_config.get("weixin_oc_base_url", "https://ilinkai.weixin.qq.com")).rstrip("/")
        ...
        self._login_session: WeixinLoginSession | None = None
        self._sync_buf = ""
        ...
        self.account_id = str(platform_config.get("weixin_oc_account_id", "")).strip() or None
    ...
    async def _ensure_typing_ticket(
        ...
        if state.ticket and state.ticket_context_token == context_token and state.refresh_after > now:
            return state.ticket
    ...
    async def _send_typing_state(
        ...
        *,
        ...
        if not self._is_successful_api_payload(payload):
            raise RuntimeError(f"sendtyping failed for {user_id}: {self._format_api_error(payload)}")
    ...
    async def _run_typing_keepalive(self, user_id: str) -> None:
        ...
        try:
            ...
            if state is not None:
                async with state.lock:
                    ...
                    restart_needed = bool(state.owners) and not self._shutdown_event.is_set()
            logger.warning(
        ...
        async with state.lock:
            if not state.owners or state.keepalive_task is not None:
                return
            state.keepalive_task = asyncio.create_task(self._run_typing_keepalive(user_id))
    ...
    async def stop_typing(self, user_id: str, owner_id: str) -> None:
        ...
        async with state.lock:
            ...
            if ticket and (state.cancel_task is None or state.cancel_task.done()):
                state.cancel_task = asyncio.create_task(self._delayed_cancel_typing(user_id, ticket))
    ...
    async def _cleanup_typing_tasks(self) -> None:
        ...
        for user_id, state in list(self._typing_states.items()):
            if state.ticket and (state.owners or state.keepalive_task is not None or state.cancel_task is not None):
    ...
    def _normalize_context_tokens(self, raw_context_tokens: Mapping[object, object]) -> dict[str, str]:
    ...
    def _is_login_session_valid(self, login_session: WeixinLoginSession | None) -> bool:
    ...
    def _save_inbound_media(
        ...
        *,
        ...
        target = self._resolve_inbound_media_dir() / f"{prefix}_{uuid.uuid4().hex}_{stem}{suffix}"
    ...
    async def _resolve_media_file_path(self, segment: Image | Video | File) -> Path | None:
        try:
    ...
    async def _send_media_segment(
        ...
        if not self.token:
            logger.warning("weixin_oc(%s): missing token, skip media send", self.meta().id)
            return False
        ...
        file_name = segment.name if isinstance(segment, File) and segment.name else media_path.name
        try:
    ...
    async def _start_login_session(self) -> WeixinLoginSession:
    ...
    async def _poll_qr_status(self, login_session: WeixinLoginSession) -> None:
    ...
    def _cache_recent_message(
        ...
        *,
        ...
        if not session_id or not message_id:
            return
        resolved_timestamp_ms = timestamp_ms if timestamp_ms is not None else timestamp * 1000
        ...
        cache.append(
            WeixinOCRecentMessage(
                ...
                message_kind=message_kind or self._infer_message_kind_from_components(components),
            )
        )
    ...
    async def _build_reply_component_from_ref(
        ...
        *,
        ...
        try:
            quoted_item_type = int(quoted_item_type_raw) if quoted_item_type_raw not in (None, "") else None
        except (TypeError, ValueError):
        ...
        try:
            ref_create_time_ms = int(ref_create_time_ms_raw) if ref_create_time_ms_raw not in (None, "") else None
        except (TypeError, ValueError):
        ...
        reply_sender_id_raw = matched_message.sender_id if matched_message is not None else quoted_sender_id_raw
        normalized_reply_sender_id = self._normalize_reply_sender_id(reply_sender_id_raw)
        reply_sender_id = normalized_reply_sender_id if normalized_reply_sender_id else reply_sender_id_raw
        reply_sender_nickname = matched_message.sender_nickname if matched_message is not None else quoted_sender_id_raw
        reply_time = (
            matched_message.timestamp
            if matched_message is not None
            else (int(ref_create_time_ms / 1000) if isinstance(ref_create_time_ms, int) else int(time.time()))
        )
    ...
    async def _item_list_to_components(self, item_list: list[dict[str, Any]] | None) -> list[Any]:
        if not item_list:
            return []
        parts: list[Any] = []
        for item in item_list:
    ...
    async def _send_to_session(self, user_id: str, text: str, _components: list[Any] | None = None) -> bool:
        if not text:
    ...
    def get_stats(self) -> dict:
        ...
        stat["weixin_oc"] = {
            ...
            "qrcode_img_content": login_session.qrcode_img_content if login_session else None,
