...
class MattermostClient:
    ...
    async def get_json(self, path: str) -> dict[str, Any]:
        ...
        async with session.get(url, headers=self._headers()) as resp:
            if resp.status >= 400:
                ...
                raise RuntimeError(f"Mattermost GET {path} failed: {resp.status} {body}")
    ...
    async def post_json(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        async with session.post(url, headers=self._headers(), json=payload) as resp:
            if resp.status >= 400:
                body = await resp.text()
                raise RuntimeError(f"Mattermost POST {path} failed: {resp.status} {body}")
            data = await resp.json()
            if not isinstance(data, dict):
    ...
    async def download_file(self, file_id: str) -> bytes:
        ...
        async with session.get(url, headers=self._auth_headers()) as resp:
            if resp.status >= 400:
                body = await resp.text()
                raise RuntimeError(f"Mattermost download file {file_id} failed: {resp.status} {body}")
    ...
    async def upload_file(
        ...
        async with session.post(url, headers=self._auth_headers(), data=form) as resp:
            if resp.status >= 400:
                body = await resp.text()
                raise RuntimeError(f"Mattermost upload file failed: {resp.status} {body}")
            data = await resp.json()
    ...
    async def parse_post_attachments(
        ...
        for file_id in file_ids:
            try:
                ...
                logger.warning("Mattermost fetch attachment failed %s: %s", file_id, exc)
                continue
