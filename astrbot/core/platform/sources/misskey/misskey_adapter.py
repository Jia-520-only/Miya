...
@register_platform_adapter("misskey", "Misskey 平台适配器", support_streaming_message=False)
class MisskeyPlatformAdapter(Platform):
    ...
    async def send_by_session(
        ...
        try:
            ...
            has_file_components = any(
                isinstance(comp, (Comp.Image, Comp.File))
                or hasattr(comp, "convert_to_file_path")
                or hasattr(comp, "get_file")
                or any(hasattr(comp, a) for a in ("file", "url", "path", "src", "source"))
                for comp in message_chain.chain
            )
            ...
            if session_id and is_valid_room_session_id(session_id):
                ...
                if is_valid_chat_session_id(session_id):
                    ...
                    user_id_for_cache = session_id.split("%")[1] if "%" in session_id else session_id
    ...
    async def convert_message(self, raw_data: dict[str, Any]) -> AstrBotMessage:
        ...
        poll = raw_data.get("poll") or (
            raw_data.get("note", {}).get("poll") if isinstance(raw_data.get("note"), dict) else None
        )
        if poll and isinstance(poll, dict):
            self._process_poll_data(message, poll, message_parts)

        message.message_str = " ".join(part for part in message_parts if part.strip()) if message_parts else ""
        return message
