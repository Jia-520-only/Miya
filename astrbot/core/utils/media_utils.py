...
async def get_media_duration(file_path: str) -> int | None:
    ...
    try:
        ...
        logger.warning("[Media Utils] ffprobe未安装或不在PATH中，无法获取媒体时长。请安装ffmpeg: https://ffmpeg.org/")
        return None
...
async def convert_audio_to_opus(audio_path: str, output_path: str | None = None) -> str:
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            if output_path and os.path.exists(output_path):
                try:
                    ...
                    logger.debug(f"[Media Utils] 已清理失败的opus输出文件: {output_path}")
                except OSError as e:
        ...
        logger.error("[Media Utils] ffmpeg未安装或不在PATH中，无法转换音频格式。请安装ffmpeg: https://ffmpeg.org/")
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            if output_path and os.path.exists(output_path):
                try:
                    os.remove(output_path)
                    logger.debug(f"[Media Utils] 已清理失败的{output_format}输出文件: {output_path}")
                except OSError as e:
                    logger.warning(f"[Media Utils] 清理失败的{output_format}输出文件时出错: {e}")
        ...
        logger.error("[Media Utils] ffmpeg未安装或不在PATH中，无法转换视频格式。请安装ffmpeg: https://ffmpeg.org/")
        raise Exception("ffmpeg not found")
    except Exception as e:
