...
def get_shiki_runtime() -> str:
    runtime_path = Path(__file__).resolve().parent / "template" / "shiki_runtime.iife.js"
        ...
        try:
            async with (
                ...
                if resp.status == 200:
                    ...
                    self.endpoints = [ep.get("url") for ep in all_endpoints if ep.get("active") and ep.get("url")]
                    logger.info(
