...
class ShowPC(BaseTool):
    ...
    def config(self) -> Dict[str, Any]:
        return {
            "name": "show_pc",
            "description": "查看和分析 TRPG 角色卡（跑团角色卡）。当用户要求'分析角色'、'查看角色卡'、'角色信息'、'PC信息'或提及具体角色名称时使用此工具。此工具显示角色的属性、技能、装备等详细战斗信息。",
            "parameters": {
                "type": "object",
                "properties": {
                    "user_id": {
                        "type": "integer",
                        "description": "玩家QQ号。如果用户提到具体角色名但没给QQ号，先尝试用 find_member 工具查找该玩家；不填则查看请求者的角色卡"
                    }
                }
            }
        }
