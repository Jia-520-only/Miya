...
class QQLike(BaseTool):
    ...
    def config(self) -> Dict[str, Any]:
        return {
            "name": "qq_like",
            "description": "给指定QQ号点赞。支持以下场景：1. 用户说'给我点赞'、'帮我点个赞'时，给当前用户点赞；2. 用户明确指定QQ号时使用指定号码；3. 用户通过@提及其他用户（如'给@苦玄点赞'）时，context.at_list中已包含被@用户的QQ号，直接使用即可，无需调用find_member。注意：QQ每次最多点赞10次，每日有上限。如果消息中@了多个用户，需要分别调用此工具给每个人点赞。",
            "parameters": {
                "type": "object",
                "properties": {
                    "target_user_id": {
