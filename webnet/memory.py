...
class MemoryNet:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # 注册 M-Link 节点
            self.mlink.register_node("memory", ["conversation_history", "undefined_memory", "memory_flow"])
    ...
    async def _handle_export(self, message: Message) -> Message:
        ...
        try:
            ...
            # 导出数据
            export_files = await self.memory_system.export_all(Path(output_dir) if output_dir else None)
    ...
    async def get_all_conversations(self) -> List[Dict]:
        ...
        通过 get_all_session_ids 扫描所有持久化会话，
        聚合返回完整的对话列表。
        ...
        try:
            ...
            for session_id in session_ids:
                try:
                    messages = await self.conversation_history.get_history(session_id, limit=self.max_context)
                    for msg in messages:
    ...
    async def store(self, memory_data: Dict) -> None:
        ...
        try:
            ...
            if memory_type == "conversation":
                ...
                # 添加到 Undefined 记忆
                await self.undefined_memory.add(fact=memory_data.get("fact", ""), tags=memory_data.get("tags", []))
    ...
    async def search_all_conversations(self, query: str, limit: int = 20, user_id: str = None) -> List[Dict]:
        """
        ...
        try:
            ...
            for pattern, tag in date_patterns:
                ...
                for match in matches:
                    ...
                    await self.undefined_memory.add(fact, tags=[fact_tag, "auto_extracted", "日期记忆"])
                    extracted_count += 1
            ...
            if memory_type == "tide":
                ...
                if self.memory_system.neo4j_client:
                    ...
                    if all([subject, predicate, obj]):
                        rel_id = neo4j.create_memory_quintuple(subject, predicate, obj, context, emotion)
                        logger.info(f"[MemoryNet] 已存储知识图谱关系: {rel_id}")

        except Exception as e:
            logger.error(f"[MemoryNet] 存储到 MemoryEngine 失败: {e}")

    async def retrieve_from_memory_engine(self, memory_type: str = "tide", query: str = None) -> List[Dict]:
        """
        从 MemoryEngine 检索记忆
    ...
    async def compress_conversation_to_tide(self, session_id: str, recent_count: int = 10):
        """
        将对话历史压缩为潮汐记忆
        ...
        try:
            ...
            # 更新对话历史（只保留最近的）
            # 注意：这里需要实现一个截断方法
            logger.info(f"[MemoryNet] 已压缩对话 {session_id}: {len(old_messages)} 条 -> 1 条摘要")
