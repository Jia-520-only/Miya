...
class NetManager:
    ...
    def register(self, net_id: str, net_type: str, config: Dict = None) -> bool:
        """注册子网"""
    ...
    def get_active_networks(self) -> List[NetNode]:
        """获取活跃子网"""
        return [node for node in self.networks.values() if node.status == "active"]
    ...
    def get_network_stats(self) -> Dict:
        ...
        type_counts = {net_type: len(net_ids) for net_type, net_ids in self.type_index.items()}
