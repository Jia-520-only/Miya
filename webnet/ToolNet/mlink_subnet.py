...
class PerceptionLayer:
    ...
    async def preprocess(self, tool_name: str, args: Dict[str, Any], context: ToolContext) -> PerceptionResult:
        """
        预处理工具请求
...
class MLinkToolSubnet:
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any], context: ToolContext) -> str:
        ...
        if self.perception_layer:
            perception_result = await self.perception_layer.preprocess(tool_name, args, context)
    ...
    async def execute_tool_batch(self, tool_calls: List[Dict[str, Any]], context: ToolContext) -> List[str]:
        """
        批量执行工具
