...
class DataAnalyzer(BaseTool):
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "time_column": {"type": "string", "description": "时间列名（可选，用于时间趋势分析）"},
                        "method": {
                            "type": "string",
                            "description": "检测方法: iqr, zscore, isolation_forest（用于异常检测）",
                            "enum": ["iqr", "zscore", "isolation_forest"],
                        },
                        "threshold": {"type": "number", "description": "阈值参数（默认1.5）"},
                        "features": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "特征列列表（用于聚类分析）",
                        },
                        "n_clusters": {"type": "integer", "description": "聚类数量（默认3）"},
                        "correlation_method": {
                            "type": "string",
    ...
    def analyze_trends(self, df: pd.DataFrame, value_column: str, time_column: str = None) -> Dict[str, Any]:
        """
        趋势分析
        ...
        # 计算趋势指标
        trend_analysis = {
            "数据范围": {"最小值": values.min(), "最大值": values.max(), "极差": values.max() - values.min()},
            "集中趋势": {
                "平均值": values.mean(),
                "中位数": values.median(),
        ...
        if time_column and time_column in df.columns:
            ...
            trend_analysis["时间趋势"] = self._analyze_time_trend(df_sorted[time_column], df_sorted[value_column])
    ...
    def _analyze_time_trend(self, time_data: pd.Series, value_data: pd.Series) -> Dict[str, Any]:
        """分析时间序列趋势"""
        # 计算环比增长率
        growth_rates = []
    ...
    def _analyze_distribution(self, values: pd.Series) -> Dict[str, Any]:
        ...
        return {
            "四分位数": {"Q1 (25%)": q1, "Q2 (50%)": values.median(), "Q3 (75%)": q3, "IQR": iqr},
            "偏度": skewness,
            "偏度描述": skew_desc,
            "峰度": kurtosis,
            "峰度描述": kurt_desc,
    ...
    def detect_anomalies(
        self, df: pd.DataFrame, value_column: str, method: str = "iqr", threshold: float = 1.5
    ) -> pd.DataFrame:
        """
        检测异常值
    ...
    def generate_insights(self, df: pd.DataFrame, analysis_results: Dict[str, Any]) -> List[str]:
        ...
        if len(anomalies) > 0:
            ...
            insights.append(
                f"🚨 检测到 {anomaly_count} 个异常值（占 {anomaly_ratio:.1f}%），建议核实这些数据点的准确性。"
            )
        ...
        if len(numeric_cols) >= 2:
            ...
            for i in range(len(corr_matrix.columns)):
                for j in range(i + 1, len(corr_matrix.columns)):
                    ...
                    if corr > 0.8:
                        high_corr_pairs.append(f"{corr_matrix.columns[i]} 和 {corr_matrix.columns[j]}")
    ...
    def cluster_analysis(self, df: pd.DataFrame, features: List[str], n_clusters: int = 3) -> Dict[str, Any]:
        """
        聚类分析
        ...
        return {"clustered_data": result_df, "cluster_stats": cluster_stats, "cluster_centers": kmeans.cluster_centers_}
    ...
    def comprehensive_analysis(self, df: pd.DataFrame, value_column: str, time_column: str = None) -> Dict[str, Any]:
        """
        综合分析报告
...
def analyze_data(
    data: pd.DataFrame, analysis_type: str = "comprehensive", value_column: str = None, config: Dict[str, Any] = None
) -> Dict[str, Any]:
    """
    数据分析的统一接口
    ...
    if analysis_type == "trend":
        ...
        anomalies = analyzer.detect_anomalies(
            data, value_column, method=config.get("method", "iqr"), threshold=config.get("threshold", 1.5)
        )
