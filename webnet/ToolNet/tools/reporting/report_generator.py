...
class ReportGenerator:
    ...
    def generate_research_report(self, research_data: Dict[str, Any], report_config: Dict[str, Any] = None) -> str:
        """
        生成结构化调研报告
...
{
            chr(10).join(
                f"- {category}: {len(research_data.get('调研内容', {}).get(category, []))}条信息"
...
{chr(10).join(f"- {item.get('标题', item.get('内容', ''))[:100]}" for item in items[:3])}
...
{
            chr(10).join(
                f"| {i + 1} | {source[:50]} | {'高' if '.gov' in source or '.edu' in source else '中'} |"
...
```json
{json.dumps({k: v for k, v in research_data.items() if k != "深度分析"}, ensure_ascii=False, indent=2)[:5000]}...
```
...
"""
    ...
    def generate_ppt_outline(self, research_data: Dict[str, Any], config: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        生成PPT大纲
        ...
        outline = {"主题": theme, "总页数": slide_count, "幻灯片": []}
        ...
        # 第2页：目录
        outline["幻灯片"].append({"页码": 2, "类型": "目录", "内容": ["调研背景", "核心发现", "详细分析", "结论建议"]})
        ...
        outline["幻灯片"].append({"页码": 4, "类型": "内容页", "标题": "核心发现", "要点": key_findings[:6]})
        ...
        for i in range(9, 13):
            outline["幻灯片"].append(
                {"页码": i, "类型": "数据页", "标题": f"数据展示 {i - 8}", "说明": "建议根据实际数据生成图表"}
            )
        ...
        # 第15页：致谢/结束页
        outline["幻灯片"].append({"页码": 15, "类型": "结束页", "标题": "谢谢", "副标题": "感谢您的聆听"})

        return outline

    def export_ppt_content(self, outline: Dict[str, Any], output_format: str = "markdown") -> str:
        """
        导出PPT内容
...
def generate_report_command(
    research_data: Dict[str, Any], output_type: str = "report", config: Dict[str, Any] = None
) -> Dict[str, Any]:
    """
    报告生成命令统一接口
    ...
    try:
        ...
        return {"success": True, "输出路径": output_path, "输出类型": output_type, "内容长度": len(content)}
