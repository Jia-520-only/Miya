...
class InvoiceParser:
    ...
    def parse_invoice_from_text(self, text: str) -> Dict[str, Any]:
        ...
        Returns:
            解析的发票信息字典
        """
        invoice_data = {"解析状态": "失败", "提取字段": {}, "置信度": 0}
    ...
    def parse_invoice_from_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 图片文件（需要OCR）
                ...
                return {"error": "图片文件需要OCR支持，请先转换为文本", "建议": "使用其他工具识别图片中的文字后再解析"}
    ...
    def _extract_text_from_pdf(self, file_path: str) -> str:
        """从PDF提取文本（复用 file_analysis 唯一实现）"""
        try:
            from webnet.ToolNet.tools.file_analysis.file_analyzer import extract_pdf_text
    ...
    def create_expense_table(self, invoices: List[Dict[str, Any]], output_path: str) -> bool:
        """
        创建报销表格
    ...
    def batch_parse_invoices(self, directory: str, patterns: List[str] = None) -> Dict[str, Any]:
        """
        批量解析目录中的发票
        ...
        # 解析每个文件
        results = {"总文件数": len(all_files), "成功解析": 0, "失败解析": 0, "发票列表": []}

        for file_path in all_files:
            result = self.parse_invoice_from_file(str(file_path))

            if "error" not in result:
                results["发票列表"].append({"文件": file_path.name, **result})
                results["成功解析"] += 1
            else:
                results["发票列表"].append({"文件": file_path.name, "错误": result.get("error", "未知错误")})
                results["失败解析"] += 1
    ...
    def generate_report(self, invoices: List[Dict[str, Any]], report_path: str) -> bool:
        """
        生成报销报告
...
def process_invoice_command(
    files: List[str], operations: List[str], output_path: str = "./expense_report.xlsx"
) -> Dict[str, Any]:
    """
    执行发票处理命令
    ...
    results = {"成功": True, "处理文件数": 0, "发票数据": [], "输出文件": []}
