...
class PDFDocxProcessor:
    ...
    def _load_pdf(self, file_path: str) -> Dict[str, Any]:
        """加载PDF文件（文本提取复用 file_analysis 唯一实现）"""
        ...
        for i, page_text in enumerate(page_texts):
            page_contents.append({"页码": i + 1, "内容": page_text, "字符数": len(page_text)})
            full_text += page_text + "\n"
        ...
        return {
            ...
            "元数据": {
                ...
                "创建日期": metadata.get("creation_date", ""),
                "修改日期": metadata.get("mod_date", ""),
            },
            "完整文本": full_text,
            "分页内容": page_contents,
            "总字符数": len(full_text),
    ...
    def _load_docx(self, file_path: str) -> Dict[str, Any]:
        ...
        for i, para in enumerate(doc.paragraphs):
            ...
            if text:
                paragraphs.append({"段落": i + 1, "内容": text, "字符数": len(text)})
                full_text += text + "\n"
        ...
        for i, table in enumerate(doc.tables):
            ...
            tables.append({"表格": i + 1, "行数": len(table.rows), "列数": len(table.columns), "数据": table_data})
    ...
    def extract_keywords(self, content: str, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        for word, freq in sorted_words[:top_n]:
            keywords.append({"关键词": word, "频次": freq, "重要度": freq / len(words) * 100})
    ...
    def _analyze_structure(self, doc_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if doc_type == "PDF":
            return {"类型": "PDF文档", "结构特点": [f"共 {doc_data.get('总页数')} 页"]}
        elif doc_type == "Word":
            return {
                "类型": "Word文档",
                "结构特点": [f"包含 {doc_data.get('总段落数')} 个段落", f"包含 {doc_data.get('总表格数')} 个表格"],
    ...
    def extract_tables_from_pdf(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        for page_num, page in enumerate(reader.pages):
            ...
            if table_lines:
                tables.append({"页码": page_num + 1, "数据": table_lines})
    ...
    def search_in_document(self, file_path: str, keyword: str) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            results.append({"位置": pos, "上下文": context, "关键词位置": pos - context_start})
...
def process_document_command(file_path: str, operations: List[str], output_path: str = None) -> Dict[str, Any]:
    """
    执行文档处理命令
    ...
    results = {"file_path": file_path, "operations": [], "results": {}}
