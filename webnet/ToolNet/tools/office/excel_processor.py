...
class ExcelProcessor:
    ...
    def cross_table_match(
        self, df1: pd.DataFrame, df2: pd.DataFrame, join_key1: str, join_key2: str, join_type: str = "inner"
    ) -> pd.DataFrame:
        """
        跨表格数据匹配
        ...
        try:
            result = pd.merge(
                df1, df2, left_on=join_key1, right_on=join_key2, how=join_type, suffixes=("_left", "_right")
            )
    ...
    def generate_statistics(self, df: pd.DataFrame, columns: List[str] = None) -> Dict[str, Any]:
        ...
        stats = {"总行数": len(df), "总列数": len(df.columns), "数值列统计": {}}
    ...
    def group_and_aggregate(
        self, df: pd.DataFrame, group_by: str, agg_column: str, agg_func: str = "sum"
    ) -> pd.DataFrame:
        """
        分组聚合
    ...
    def add_calculated_column(
        self, df: pd.DataFrame, new_col_name: str, formula: str, columns_map: Dict[str, str] = None
    ) -> pd.DataFrame:
        """
        添加计算列
...
# 工具函数
def process_excel_command(file_path: str, operations: List[Dict[str, Any]], output_path: str) -> Dict[str, Any]:
    """
    执行Excel处理命令
    ...
    for op in operations:
        ...
        if op_type == "clean":
            ...
            if df2 is not None:
                df = processor.cross_table_match(df, df2, op.get("key1"), op.get("key2"), op.get("join_type", "inner"))
                stats["operations_performed"].append(f"跨表匹配: {op.get('key1')} <-> {op.get('key2')}")

        elif op_type == "aggregate":
            df = processor.group_and_aggregate(df, op.get("group_by"), op.get("agg_col"), op.get("func", "sum"))
            stats["operations_performed"].append("分组聚合")

        elif op_type == "add_column":
            df = processor.add_calculated_column(df, op.get("name"), op.get("formula"), op.get("columns_map"))
    ...
    return {"success": success, "stats": stats, "output_file": output_path}
