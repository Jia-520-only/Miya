...
class GetRecentMessagesTool(BaseTool):
    ...
    def config(self) -> Dict[str, Any]:
        return {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "chat_id": {"type": "integer", "description": "会话ID（群号或用户QQ号）。不指定则使用当前会话"},
                    "chat_type": {
                        "type": "string",
                    ...
                    "sender_id": {"type": "integer", "description": "发送者QQ号过滤"},
                },
                "required": [],
            },
        }
    ...
    async def execute(self, args: Dict[str, Any], context: ToolContext) -> str:
        ...
        if context.memory_net and hasattr(context.memory_net, "get_recent_conversations"):
            try:
                memories = await context.memory_net.get_recent_conversations(user_id=chat_id, limit=count)
