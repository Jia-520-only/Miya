...
class CreateScheduleTaskTool(BaseTool):
    ...
    async def execute(self, args: Dict[str, Any], context: ToolContext) -> str:
        ...
        try:
            ...
            # ── 记录任务所属平台（调度器按此跨平台分发定时消息）──
            task_platform = getattr(context, "platform", None)
            if not task_platform:
                adapter = getattr(context, "platform_adapter", None)
                task_platform = getattr(adapter, "platform_id", None) if adapter else None
            if not task_platform:
                task_platform = "aiocqhttp"
