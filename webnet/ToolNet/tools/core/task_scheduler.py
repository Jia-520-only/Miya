...
class TaskBuilder:
    ...
    def interval(self, seconds: int = 0, minutes: int = 0, hours: int = 0, days: int = 0) -> "TaskBuilder":
        ...
        self.trigger_args = {"seconds": seconds, "minutes": minutes, "hours": hours, "days": days}
        return self

    def cron(
        self, minute: str = "*", hour: str = "*", day: str = "*", month: str = "*", day_of_week: str = "*"
    ) -> "TaskBuilder":
        """设置Cron触发"""
        self.trigger_type = "cron"
        self.trigger_args = {"minute": minute, "hour": hour, "day": day, "month": month, "day_of_week": day_of_week}
        return self

    def once(self, run_date: datetime) -> "TaskBuilder":
        """设置单次触发"""
        self.trigger_type = "date"
        self.trigger_args = {"run_date": run_date}
        return self
...
# 便捷函数
def schedule_once(scheduler: TaskScheduler, func: Callable, run_date: datetime, name: str = None, **kwargs) -> str:
    """
    调度单次任务
...
def schedule_interval(
    scheduler: TaskScheduler, func: Callable, interval_seconds: int, name: str = None, **kwargs
) -> str:
    """
    调度间隔任务
...
def schedule_cron(scheduler: TaskScheduler, func: Callable, cron_expression: str, name: str = None, **kwargs) -> str:
    """
    调度Cron任务
    ...
    task = ScheduledTask(
        ...
        func=func,
        trigger_type="cron",
        trigger_args={"minute": minute, "hour": hour, "day": day, "month": month, "day_of_week": day_of_week},
        kwargs=kwargs,
    )
