...
class SystemMonitor:
    ...
    def get_processes(self, limit: int = 10, sort_by: str = "cpu") -> List[ProcessInfo]:
        ...
        for proc in psutil.process_iter(
            ["pid", "name", "status", "cpu_percent", "memory_percent", "num_threads", "create_time"]
        ):
            try:
                proc_info = proc.info
        ...
        for rule in self.alert_rules:
            ...
            if triggered:
                if rule.name not in self.alert_states:
                    self.alert_states[rule.name] = {"first_triggered": datetime.now(), "triggered_count": 0}
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cpu": {"current": current.cpu_percent, "trend": self.get_trend("cpu_percent")},
            "memory": {
            ...
            "network": {"sent_gb": current.network_sent / (1024**3), "recv_gb": current.network_recv / (1024**3)},
...
class ProcessMonitor:
    ...
    def get_average_cpu(self, duration: int = 300) -> float:
        """获取平均CPU使用率"""
        cutoff_time = datetime.now() - timedelta(seconds=duration)
        relevant = [p for p in self.history if datetime.fromisoformat(p["timestamp"]) >= cutoff_time]
...
if __name__ == "__main__":
    ...
    # 添加告警规则
    monitor.add_alert_rule(AlertRule(name="CPU高告警", metric="cpu_percent", threshold=80.0, operator=">"))
