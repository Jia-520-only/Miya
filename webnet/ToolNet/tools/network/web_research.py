...
class WebResearcher:
    ...
    async def research_topic(self, topic: str, research_type: str = "general", depth: int = 2) -> Dict[str, Any]:
        """
        调研主题
        ...
        if research_type == "general":
            queries = [f"{topic} 概念 定义", f"{topic} 发展趋势", f"{topic} 应用场景", f"{topic} 挑战与机遇"]
            ...
            queries = [f"{topic} 竞品对比分析", f"{topic} 优劣势分析", f"{topic} 市场份额", f"{topic} 用户评价"]
        elif research_type == "product":
            queries = [f"{topic} 产品功能特性", f"{topic} 技术架构", f"{topic} 使用案例", f"{topic} 定价策略"]
        else:
            queries = [topic]
    ...
    async def research_competitors(self, company_name: str) -> Dict[str, Any]:
        """
        调研竞品
    ...
    def _categorize_information(self, query: str, result: Dict[str, Any]) -> str:
        """分类信息（依据查询关键词）"""
        # 根据查询类型分类
    ...
    async def _deep_analyze(self, content: Dict[str, List[Dict]]) -> Dict[str, Any]:
        """深度分析调研内容"""
        analysis = {"信息覆盖度": {}, "关键发现": [], "信息缺口": []}
            ...
            for item in items:
                ...
                if relevance >= 5:  # 高相关性结果
                    analysis["关键发现"].append(
                        {"类别": category, "来源": item.get("来源", ""), "标题": title, "重要性": "高"}
                    )
    ...
    def _generate_competitor_summary(self, competitor_data: Dict[str, Any]) -> str:
        """生成竞品总结"""
        summary_parts = []
        ...
        if competitor_data["用户评价"]:
            positive = sum(
                1
                for item in competitor_data["用户评价"]
                if any(kw in item.get("snippet", "") for kw in ["好", "优秀", "推荐"])
            )
            negative = sum(
                1
                for item in competitor_data["用户评价"]
                if any(kw in item.get("snippet", "") for kw in ["差", "不好", "不推荐"])
            )
    ...
    def generate_research_report(self, research_data: Dict[str, Any], output_path: str) -> bool:
        """
        生成调研报告
...
- **内容摘要**: {content[:200]}{"..." if len(content) > 200 else ""}
...
async def research_command(
    topic: str, research_type: str = "general", options: Dict[str, Any] = None
) -> Dict[str, Any]:
    """
    调研命令统一接口
    ...
    if research_type == "competitor":
        ...
        results = await researcher.research_topic(topic, research_type, options.get("depth", 2))
