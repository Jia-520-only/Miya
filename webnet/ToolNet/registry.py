...
class ToolRegistry:
    ...
    def load_all_tools(self):
        ...
        self._load_cross_terminal_tools()  # 新增：加载跨端工具
    ...
    def _load_cross_terminal_tools(self):
        """加载跨端工具"""
        try:
            from webnet.CrossTerminalNet.tools.cross_terminal import CrossTerminalTools
            from webnet.CrossTerminalNet.hub import get_cross_terminal_hub

            # 获取跨端Hub实例
            hub = get_cross_terminal_hub()
            tools = CrossTerminalTools(hub)

            # 手动注册工具函数
            from webnet.tools.base import BaseTool

            # 创建跨端工具实例
            self._register_cross_terminal_tool("send_to_desktop", tools.send_to_desktop)
            self._register_cross_terminal_tool("execute_on_desktop", tools.execute_on_desktop)
            self._register_cross_terminal_tool("sync_state", tools.sync_state)
            self._register_cross_terminal_tool("send_to_terminal", tools.send_to_terminal)
            self._register_cross_terminal_tool("send_to_qq", tools.send_to_qq)
            self._register_cross_terminal_tool("send_to_web", tools.send_to_web)

            self.logger.info("跨端工具已注册")
        except Exception as e:
    ...
    def _register_cross_terminal_tool(self, name: str, func):
        ...
        tool_defs = {
            "send_to_desktop": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "notification": {"type": "boolean", "description": "是否显示通知", "default": True}
                    },
                    "required": ["content"]
                }
            },
            "execute_on_desktop": {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "command": {"type": "string", "description": "要执行的Windows命令"}
                    },
                    "required": ["command"]
                }
            },
            ...
            "send_to_qq": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "content": {"type": "string", "description": "要发送的消息内容"},
                        "target_qq": {"type": "string", "description": "目标QQ号，可选"}
                    },
                    "required": ["content"]
                }
            },
        ...
        config = tool_defs.get(name, {"name": name, "description": "跨端工具", "parameters": {"type": "object", "properties": {}}})
