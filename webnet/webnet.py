...
def _jwt_secret_key() -> str:
    """JWT 签名密钥 — 从 .env 读取，未配置时使用默认值"""
    try:
        from config.config_utils import get_api_key

        return get_api_key("MIYA_SECRET_KEY", "miya_secret_key")
    except Exception:
        return "miya_secret_key"
...
class WebNet:
    ...
    async def get_blog_posts(
        self, page: int = 1, per_page: int = 10, category: Optional[str] = None, tag: Optional[str] = None
    ) -> Dict[str, Any]:
        """获取博客列表"""
    ...
    async def create_blog_post(
        self, title: str, content: str, author: str, category: str, tags: List[str], published: bool = True
    ) -> Dict[str, Any]:
        """创建博客"""
        conn = sqlite3.connect(self.blog_store.db_path)
        cursor = conn.cursor()
    ...
    async def login_user(self, username: str, password: str) -> Dict[str, Any]:
        ...
        if not jwt_available:
            ...
            return {"success": False, "message": "JWT库未安装，请运行: pip install pyjwt"}
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "personality": {
                ...
                "vectors": {"warmth": 0.8, "logic": 0.7, "creativity": 0.6, "empathy": 0.9, "resilience": 0.7},
            },
            "emotion": {
                "primary": "calm",
                "intensity": 0.5,
                "state": "balanced",
                "current": {"joy": 0.6, "sadness": 0.1, "anger": 0.1, "fear": 0.1, "surprise": 0.1},
            },
            "memory_stats": {"tide_count": 0, "longterm_count": 0},
            "stats": {"total_visits": 0, "total_posts": 0, "total_users": 1},
            "timestamp": datetime.utcnow().isoformat(),
        }
