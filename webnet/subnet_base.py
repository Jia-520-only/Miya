...
class BaseSubnet(ABC):
    ...
    def __init__(self, config: Optional[SubnetConfig] = None):
        ...
        self.logger.info(f"{self.config.subnet_name} 子网已启动，已加载 {len(self.tools)} 个工具")
    ...
    def get_all_tools(self) -> List[Dict[str, Any]]:
        ...
        """
        return [
            {"name": name, "config": getattr(tool, "config", {}), "subnet": self.config.subnet_name}
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        success_rate = self.config.success_calls / self.config.total_calls * 100 if self.config.total_calls > 0 else 0
...
class ToolRegistrySubnet(BaseSubnet):
    ...
    def _create_default_config(self) -> SubnetConfig:
        return SubnetConfig(subnet_name="ToolRegistrySubnet", subnet_id="subnet.tool_registry", version="1.0.0")
