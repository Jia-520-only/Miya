...
class IoTNet:
    ...
    def __init__(self):
        self.net_id = "iot_net"
        self.capabilities = ["device_control", "device_monitoring", "automation"]
    ...
    def register_device(self, device_id: str, device_type: str, info: Dict = None) -> bool:
        """注册设备"""
        if not info:
            info = {}

        self.devices[device_id] = {"type": device_type, "info": info, "registered_at": self._get_timestamp()}

        # 初始化设备状态
        self.device_status[device_id] = {"online": False, "status": "idle", "attributes": {}}
    ...
    def get_all_devices(self, device_type: str = None) -> List[Dict]:
        ...
        for device_id, device_info in self.devices.items():
            if device_type is None or device_info.get("type") == device_type:
                devices.append({"id": device_id, **device_info, "status": self.device_status.get(device_id, {})})
        return devices
    ...
    def process_request(self, request: Dict) -> Dict:
        ...
        if req_type == "register_device":
            success = self.register_device(request.get("device_id"), request.get("device_type"), request.get("info"))
            return {"success": success}
        elif req_type == "control_device":
            success = self.control_device(request.get("device_id"), request.get("command"), request.get("parameters"))
            return {"success": success}
