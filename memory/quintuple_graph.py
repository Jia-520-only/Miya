...
def _load_config():
    ...
    try:
        ...
        if hasattr(config, 'grag'):
            GRAG_ENABLED = getattr(config.grag, 'enabled', True)
            NEO4J_URI = getattr(config.grag, 'neo4j_uri', None)
            NEO4J_USER = getattr(config.grag, 'neo4j_user', None)
            NEO4J_PASSWORD = getattr(config.grag, 'neo4j_password', None)
            NEO4J_DATABASE = getattr(config.grag, 'neo4j_database', 'neo4j')
...
def get_graph():
    ...
    if _graph is None:
        ...
        try:
            from core.config import config
            grag_enabled = getattr(config.grag, 'enabled', True) if hasattr(config, 'grag') else True
            neo4j_uri = getattr(config.grag, 'neo4j_uri', None) if hasattr(config, 'grag') else None
            neo4j_user = getattr(config.grag, 'neo4j_user', None) if hasattr(config, 'grag') else None
            neo4j_password = getattr(config.grag, 'neo4j_password', None) if hasattr(config, 'grag') else None
            neo4j_database = getattr(config.grag, 'neo4j_database', 'neo4j') if hasattr(config, 'grag') else 'neo4j'
