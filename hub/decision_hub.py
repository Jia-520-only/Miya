...
class DecisionHub:
    ...
    async def process_perception_cross_platform(self, message: Message) -> Optional[str]:
        ...
        if self.auth_subnet and not is_terminal_agent and not is_desktop:
            try:
                ...
                if '_' in user_id and any(user_id.startswith(p + '_') for p in ['qq', 'terminal', 'web', 'desktop']):
    ...
    async def _get_conversation_context(self, session_id: str, current_input: str = "") -> List[Dict]:
        """
        获取对话历史上下文（限制token消耗）
        ...
        # 【智能判断】检测用户是否在问关于过去的问题
        # 只有这种情况下才加载更多历史，否则只加载最近3条
        needs_recall = self._check_needs_recall(current_input)
        
        # 根据是否需要回忆决定加载数量
        if needs_recall:
            # 用户在问过去的事情，加载更多历史（最多20条）
            max_messages = 20
            logger.info(f"[决策层-跨平台] 用户正在回忆过去，加载历史对话: {session_id}")
        else:
            # 正常对话，只加载最近3条
            max_messages = 3
            logger.debug(f"[决策层-跨平台] 正常对话，只加载最近3条: {session_id}")
        ...
        try:
            ...
            logger.debug(f"[决策层-跨平台] 加载对话历史: {len(recent_messages)} 条")
            ...
            logger.error(f"[决策层-跨平台] 获取对话历史失败: {e}")
            return []
    ...
    def _check_needs_recall(self, user_input: str) -> bool:
        ...
        for pattern in recall_patterns:
            if re.search(pattern, user_input):
                logger.info(f"[决策层-跨平台] 检测到回忆请求: {user_input[:30]}")
                return True
    ...
    async def _generate_response_cross_platform(self, content: str,
        ...
        try:
            ...
            conversation_context = await self._get_conversation_context(session_id, current_input=content)
    ...
    def _get_platform_specific_tools(self, platform: str) -> list:
        ...
        只返回当前平台最常用的核心工具，避免 101 个工具导致 API 500 错误
        ...
        try:
            ...
            logger.info(f"[决策层] 平台 {platform} 使用 {len(platform_schemas)} 个工具")
            return platform_schemas
            
        except Exception as e:
            logger.warning(f"[决策层] 获取平台工具失败: {e}，使用全部工具")
