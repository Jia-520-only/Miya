...
import crossTerminalService from './utils/websocket'
...
onMounted(() => {
  ...
  // 启动跨端消息轮询
  crossTerminalService.startPolling()

  // 注册跨端消息通知回调
  crossTerminalService.onNotification((notification) => {
    console.log('[CrossTerminal] 收到通知:', notification)
    // 可以在这里显示桌面通知
    if (Notification.permission === 'granted') {
      new Notification('来自其他端的消息', {
...
onUnmounted(() => {
  ...
  // 停止跨端消息轮询
  crossTerminalService.stopPolling()
})
