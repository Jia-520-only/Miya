...
import ScrollPanel from '@/components/ui/MiyaScrollPanel.vue'
...
function fitWindowHeight() {
  ...
  const desired = HEADER_HEIGHT + sessionH + captureH + contentH + BORDER
...
async function handleCapture() {
  ...
  try {
    ...
    if (result && 'permission' in result) {
      ...
      captureSources.value = sources.filter(s => !s.name.includes('NagaAgent') && !s.name.includes('Miya'))
    }
  }
  catch {
    captureSources.value = []
...
<template>
  ...
  <div
    ...
    @dragstart.prevent
  >
    <div class="ball-outer-ring" :class="{ active: isGenerating }" />
    <div class="ball-inner-ring" />
      ...
      <div v-if="showCapturePanel" ref="capturePanelRef" class="capture-panel" @pointerdown.stop>
        ...
        <div class="capture-grid">
          ...
          <div v-else-if="capturePermissionDenied" class="text-white/40 text-xs text-center py-3 col-span-2">
            需要屏幕录制权限，请前往<br>系统设置 &gt; 隐私与安全性 &gt; 屏幕录制<br>中授权 Miya
